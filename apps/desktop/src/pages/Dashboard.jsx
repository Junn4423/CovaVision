  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-slate-100 px-5 py-4"><h2 className="font-bold text-slate-900">Lượt quét gần đây</h2><Link to={ROUTES.reports} className="text-xs font-bold text-blue-600 hover:underline">Xem báo cáo</Link></div>{records.length === 0 ? <p className="p-5 text-sm text-slate-500">Chưa có bản ghi quét mặt.</p> : <div className="divide-y divide-slate-100">{records.map(record => <div key={record.id} className="flex items-center justify-between gap-3 px-5 py-3"><div><p className="text-sm font-semibold text-slate-800">{record.employee_name || record.employee_id || 'Không xác định'}</p><p className="text-xs text-slate-500">{record.camera_name || 'Camera'} · Quét mặt</p></div><span className="text-xs text-slate-400">{record.captured_at ? new Date(record.captured_at).toLocaleString('vi-VN') : '-'}</span></div>)}</div>}</section>
