...
export default function SystemSettings() {
  const [settings, setSettings] = useState({ face_match_threshold: '0.55', attendance_cooldown_seconds: '30' })
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <form onSubmit={save} className="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold">Ngưỡng khớp khuôn mặt<input type="number" min="0" max="1" step="0.01" value={settings.face_match_threshold} onChange={event => setSettings({ ...settings, face_match_threshold: event.target.value })} className="mt-1.5 w-full rounded-xl border border-slate-300 px-3 py-2.5 font-normal outline-none focus:border-blue-500" /><span className="mt-1 block text-xs font-normal text-slate-400">Giá trị càng cao càng chặt.</span></label><label className="text-sm font-semibold">Thời gian chống ghi trùng (giây)<input type="number" min="0" step="1" value={settings.attendance_cooldown_seconds} onChange={event => setSettings({ ...settings, attendance_cooldown_seconds: event.target.value })} className="mt-1.5 w-full rounded-xl border border-slate-300 px-3 py-2.5 font-normal outline-none focus:border-blue-500" /></label></div>
