...
# ==============================================================================
# CovaVision - macOS/Linux Start Script
# ==============================================================================
...
[ -x "$PROJECT_ROOT/.venv/bin/python" ] || fail "Chưa có .venv. Chạy ./scripts/macos/install.sh trước."
[ -d "$PROJECT_ROOT/apps/desktop/node_modules" ] || fail "Chưa có Electron dependencies. Chạy ./scripts/macos/install.sh trước."
...
if [ "${COVAVISION_DESKTOP_MODE:-dev}" = "installed" ]; then
  ...
  open -a "$app_name"
  say "Backend đang chạy; dùng ./scripts/macos/stop.sh để dừng backend."
  exit 0
fi
